...
export const useAuthStore = defineStore('auth', () => {
  ...
  function setLogin(p: { token: string; userId: number; username: string; role: Role; nickname?: string }) {
    token.value = p.token
  ...
  return { token, userId, username, role, nickname, tokenValue, fetchInfo, setLogin, logout }
})
